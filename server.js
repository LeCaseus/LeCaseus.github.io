...
app.get('/api/entries', (req, res) => {
  ...
  const rows = type
    ? db.prepare('SELECT * FROM entries WHERE type = ? ORDER BY created_at DESC').all(type)
...
app.post('/api/entries', (req, res) => {
  ...
  const result = db.prepare('INSERT INTO entries (type, title, body, date) VALUES (?, ?, ?, ?)')
...
app.post('/api/posts', (req, res) => {
  ...
  if (!category || !title || !body) return res.status(400).json({ error: 'category, title and body required' });
  const result = db.prepare('INSERT INTO posts (category, emoji, title, excerpt, body, date, read_time) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(category, emoji || '✦', title, excerpt || '', body, date, read_time || '');
  res.json({ id: result.lastInsertRowid });
});
